...
function compileSchemaAndValidator() {
	const openApiSchema = JSON.parse( readFileSync(
		`${ __dirname }/../../../src/MediaWiki/RestApi/specs/openapi.fragment.dereferenced.json`
	) );
...
function getMatchingSchemaPath( url, paths ) {
	...
	pathLoop: for ( const path of paths ) {
		...
		for ( const [ index, pathPart ] of pathParts.entries() ) {
			// skip comparison if pathPart is a path variable (e.g. {lexeme_id})
